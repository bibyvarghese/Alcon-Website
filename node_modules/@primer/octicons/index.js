...
for (const key of Object.keys(octicons)) {
    ...
    const htmlAttributes = (_icon, defaultOptions, options) => {
        const attributes = [];
    ...
    octicons[key].toSVG = function (options = {}) {
        const { height, width } = options;
        const naturalHeight = closestNaturalHeight(Object.keys(octicons[key].heights), height || width || DEFAULT_HEIGHT);
        return `<svg ${htmlAttributes(octicons[key], octicons[key].heights[naturalHeight].options, options)}>${octicons[key].heights[naturalHeight].path}</svg>`;
    };
}
